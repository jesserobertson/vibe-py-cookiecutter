...
def test_postgresql_generation(cookies, postgresql_context):
    ...
    assert (project_dir / "infrastructure" / "docker-compose.test.yml").exists()
    assert (project_dir / "infrastructure" / "test-data" / "init_postgresql.py").exists()
...
def test_script_imports(cookies, default_context):
    ...
    script_files = [
        "quality.py", "test.py", "build.py", 
        "dev.py", "docs.py", "test_db.py"
    ]
    
    for script_file in script_files:
        script_path = scripts_dir / script_file
        if script_path.exists():  # test_db.py only exists with database backends
            script_content = script_path.read_text()
            assert "from utils import run_command" in script_content
            assert "subprocess" not in script_content  # Ensure subprocess is not imported
...
@pytest.mark.parametrize("license_type", ["MIT", "Apache-2.0", "GPL-3.0", "BSD-3-Clause"])
...
def test_cookiecutter_json_validity(template_dir):
    ...
    required_fields = [
        ...
        "package_name", "author_name", "author_email", 
        "github_username", "version", "python_version"
    ]
    ...
    assert config["use_async"] in ["yes", "no"]
    assert config["use_logerr"] in ["yes", "no"] 
    assert config["use_hypothesis"] in ["yes", "no"]
    assert config["database_backend"] in ["none", "mongodb", "postgresql"]
    assert config["include_docker"] in ["yes", "no"]
    assert config["documentation_tool"] in ["mkdocs-material", "sphinx"]